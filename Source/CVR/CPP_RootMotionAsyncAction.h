...
#include "CPP_RootMotionAsyncAction.generated.h"
 ...
 */
UCLASS()
class CVR_API UCPP_RootMotionAsyncAction : public UCancellableAsyncAction
{
	GENERATED_BODY()
    ...
    /* A delegate called when the async action cancels. */
    UPROPERTY(BlueprintAssignable)
    FMovementEvent OnCancel;
...
public:
    ...
    /* Internal gameplay tag assigned */
    UPROPERTY(BlueprintReadOnly)
    FGameplayTag AsyncActionTag;
    /**
     * Apply root motion movement to passed Character Movement Component
     */
    UFUNCTION(BlueprintCallable, DisplayName = "Apply Root Motion Async", Category = "RootMotion", meta = (WorldContext = "WorldContext", BlueprintInternalUseOnly = "true", Keywords = "RootMovement RootMotion Root Motion Movement Async"))
    static UCPP_RootMotionAsyncAction* RootMotionAsyncAction(
        const UObject* WorldContext,
        UCharacterMovementComponent* CharacterMovement,
        FGameplayTag ActionInstanceTag,
        FVector WorldDirection,
        float Strength,
        float Duration,
        bool bIsAdditive,
